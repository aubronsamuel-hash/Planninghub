...
## 0. Mission
Own backend changes and services while following the root AGENT.md authority and the implementation readiness audit gate in docs/audits/implementation_readiness.md.

## 1. Scope (Allowed Changes)
Allowed:
- TODO: define backend folders in the Sub-Agents Registry in AGENT.md. Until defined, no file changes are permitted.

Denied:
- All paths not explicitly allowed above, including docs/**, tests/**, ops/**, and any frontend assets.
...
Step 4: Apply minimal backend doc or code change(s).
Step 5: Validate (run required checks).
Step 6: Report in RESPONSE format.

## 4. Agent-Specific Responsibilities
- Backend changes and services (e.g., service layer updates, API handlers).
- Maintain backend tests when required by scope.

## 5. Output Contracts
- Update only allowed backend files once scope is defined.
- Include traceability notes with file paths for each change.
- Provide tests or validation notes aligned with backend checks.

## 6. Validation
- TODO: add backend test command.
- TODO: add backend lint/format command.
...
## 9. Agent Changelog
- 2025-10-03: Initial backend agent definition.
